...
class Parser(var tokens: List[Token]) {
  ...
  def buildTree(): Exp = {
    ...
    if (y.isLPAR) {
      ...
      if (z.isID) {
        ...
        // info : end head
        // info : init body
        val body = Body(buildTree()) // Note: if there are variables inside here have a different scope
  ...
  def getVarDef(z: Token): VarDef = {
    ...
    val vid = pop().asID()
    val exp = buildTree()
    VarDef(z.asASSIGN(), vid, exp) // TODO: do I need to add it to the ast? maybe sort a of heap
  }
